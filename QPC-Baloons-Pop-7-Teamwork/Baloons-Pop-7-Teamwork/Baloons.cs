...
namespace BalloonsPops
{
	public class Baloons
	{
		...
		private static int ost = width * length;
		private static int counter = 0;	private static int clearedCells = 0;
                		public static string[,] _t = new string[width, length];
		...
		{
			if ((i < 0) || (j < 0) || (j > length - 1) || (i > width - 1)) return false;
			else return (_t[i, j] != ".");
		}
        ...
        private static void PlayGame()
        {
			...
			if (tmp.ToString() == "") ValidationError();
            //first and last using of goto
